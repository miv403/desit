...
class Host:
    ...
    def start(self):
        ...
        print(f"WELCOME!")
        print(f"HOST: {Const.HOSTNAME}@{self.LOCALHOST}")
    ...
    def getPubKey(self): # DONE test: getPubKey() 
        ...
        try:#ECDSA tabanlı SSH açık anahtarını (public key) üretmek ve okumak için kullanılır.
            keyFile = open(f"{Const.HOST_KEY_DIR}{Const.HOSTNAME}.pub", "r")#Anahtarın .pub uzantılı açık anahtar dosyasını okumaya çalışır.
        except FileNotFoundError:

            if not os.path.isdir(Const.HOST_KEY_DIR): # dizin oluşturulmamışsa
                os.makedirs(Const.HOST_KEY_DIR, exist_ok=True) 
                # Anahtar dizini (.config/ gibi) yoksa, oluşturulur.

            subprocess.run(["ssh-keygen",
                    "-t", "ecdsa", "-b", "521", # 521-bit ecdsa
                    "-f", f"{Const.HOST_KEY_DIR}{Const.HOSTNAME}", # key location
                    "-N", "", # empty passphrase
                    "-C", f"{Const.HOSTNAME}@{self.LOCALHOST}"]) # host@localhost
            
            # FIXME hostname yerine username işi yapılmalı
            #       rsync için username gerekiyor.

            keyFile = open(f"{Const.HOST_KEY_DIR}{Const.HOSTNAME}.pub", "r")
